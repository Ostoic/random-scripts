...
run-worker() {
	...
	ssh $USER@ci-server "cd actions-runner && config.cmd remove --local && config.cmd --url $REPO_PATH --token $WORKER_TOKEN --work _work$RANDOM --name devops$RANDOM --unattended --ephemeral && run.cmd"
}
